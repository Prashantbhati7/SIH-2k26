    ...
    try {
        ...
        if (search) {
            whereClause.OR = [
                ...
                { projectCode: isNaN(parseInt(search)) ? undefined : parseInt(search) }
            ].filter(Boolean);
        }
        if (sector)
        ...
        if (riskLevel) {
            resultProjects = resultProjects.filter(p => p.riskLevel.toLowerCase() === riskLevel.toLowerCase());
        }
        return res.json({
            total,
            page: pageNum,
...
router.get('/:projectCode', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/:projectCode/history', auth_1.authenticateToken, async (req, res) => {
    try {
        const projectCode = parseInt(req.params.projectCode, 10);
