...
router.get('/policymaker', auth_1.authenticateToken, (0, auth_1.requireRole)(['MINISTER_POLICYMAKER']), async (req, res) => {
    try {
        ...
        projects.forEach(p => {
            ...
            // Sector
            if (!sectorRiskMap[p.sector])
                sectorRiskMap[p.sector] = { total: 0, high: 0 };
            sectorRiskMap[p.sector].total += 1;
            if (risk === 'High')
                sectorRiskMap[p.sector].high += 1;
            // State
            if (!stateRiskMap[p.state])
                stateRiskMap[p.state] = { total: 0, high: 0 };
            stateRiskMap[p.state].total += 1;
            if (risk === 'High')
                stateRiskMap[p.state].high += 1;
        });
...
router.get('/manager', auth_1.authenticateToken, (0, auth_1.requireRole)(['MINISTER_POLICYMAKER', 'PROJECT_MANAGER']), async (req, res) => {
    try {
        ...
        projects.forEach(p => {
            ...
            if (pred && pred.riskLevel === 'High')
                highRisk += 1;
            p.warnings.forEach(w => {
                if (w.category.includes('COST'))
                    costWarnings += 1;
                if (w.category.includes('DELAY') || w.category.includes('SCHEDULE'))
                    delayWarnings += 1;
            });
        });
...
router.get('/field', auth_1.authenticateToken, (0, auth_1.requireRole)(['FIELD_OFFICER', 'PROJECT_MANAGER', 'MINISTER_POLICYMAKER']), async (req, res) => {
    try {
...
router.get('/contractor', auth_1.authenticateToken, (0, auth_1.requireRole)(['CONTRACTOR', 'PROJECT_MANAGER', 'MINISTER_POLICYMAKER']), async (req, res) => {
    try {
